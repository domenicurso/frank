...
// This is the piece we’ll inject into the system prompt
let currentActivityName: string = activityNames[0];

export function getCurrentActivityName() {
  return currentActivityName;
}

function pickRandomActivityName() {
  return activityNames[Math.floor(Math.random() * activityNames.length)]!;
}

export function setRandomActivity(client: Client) {
  currentActivityName = pickRandomActivityName();

  client.user?.setPresence({
    activities: [{ type: ActivityType.Playing, name: currentActivityName }],
    status: "online",
  });
}
...
export function stopActivityUpdates() {
  if (activityInterval) {
    ...
    console.log(chalk.yellow("[Presence] Activity updates stopped"));
  }
}

export function startActivityUpdates(client: Client) {
  stopActivityUpdates();
